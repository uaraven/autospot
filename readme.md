...
Autospot starts on Windows startup and constantly monitors connections. If the computer loses WiFi connection, it waits for 2 minutes for reconnection and if the new connection is not established, it creates a hotspot.

## Installation

There is no installation. Autospot is a portable exe file that just needs to be started.

Download the latest version from the [releases](https://github.com/uaraven/autospot/releases), unzip it, change the configuration according to your needs and run the program.
...
Companion device is a DIY USB dongle with a microcontroller and a display inside. Companion simplifies astrophotographer's life (already complicated without all this IT shit) by showing the current connection status of the NINA computer along with the IP address for remote desktop connection and hotspot ssid and password if hotspot is enabled.
