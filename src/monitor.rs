...
impl<'a> Monitor<'a> {
    ...
    fn run(&mut self, config_path: &Path, stop: &AtomicBool) -> Result<()> {
        ...
        while !stop.load(Ordering::Relaxed) {
            ...
            let intent = self.watchdog.evaluate(now, connected);
    ...
    /// or down if that's what the intent calls for.
    fn handle_intent(&mut self, now: Instant, status: &wifi::WifiStatus, intent: Intent) {
        match intent {
            ...
            Intent::StartHotspot { down_for } => {
                ...
                if let Err(e) = self.try_start(status) {
                    error!("could not start the hotspot: {e:#}");
                    self.watchdog.record_start_failure(now);
                }
            }
            Intent::WifiRestored {
                down_for,
    ...
    fn try_start(&mut self, wifi_status: &wifi::WifiStatus) -> Result<()> {
        ...
        match hotspot.state()? {
            ...
            _ => {
                hotspot.start(&self.cfg.hotspot)?;
                self.watchdog.record_started();
                Ok(())
            }
        }
    }
    ...
    /// Compute this tick's status and send only what changed since `self.last_status` to
    /// the companion microcontroller over serial, tagged with "t": seconds since the "s"
    /// field itself last changed -- e.g. how long Wi-Fi has been disconnected. Resolves
    /// the serial port fresh on every call (see `CompanionConn::new`) so a companion that
    /// is unplugged and replugged -- possibly under a different COM port -- is still
    /// found. Updates `self.last_status`/`self.last_state_change` on every call, even
    /// when there is no companion to send to, so both stay accurate for whenever one
    /// shows up.
    fn update_companion_status(
        ...
        let changed = new_status.diff(&self.last_status);
